...
def call_llm(messages, temperature=0.7, max_tokens=3000, _retries=3):
    client = get_groq_client()
    # Groq does not support 'system' role inside some message positions —
    # remap any 'system' messages that are not the very first to 'user'.
    remapped = []
    for i, msg in enumerate(messages):
        if msg["role"] == "system" and i > 0:
            remapped.append({"role": "user", "content": f"[SYSTEM CONTEXT]: {msg['content']}"})
        else:
            remapped.append(msg)
    for attempt in range(_retries):
        try:
            ...
            if "rate_limit" in str(e).lower() and attempt < _retries - 1:
